...
document.getElementById(‘checkinStats’).innerHTML = ` <div class="statCard blue"><div class="val">${all.length}</div><div class="lbl">Totali</div></div> <div class="statCard green"><div class="val">${free}</div><div class="lbl">Liberi</div></div> <div class="statCard red"><div class="val">${occ}</div><div class="lbl">Occupati</div></div> <div class="statCard orange"><div class="val">${Math.round(occ / all.length * 100) || 0}%</div><div class="lbl">Occupazione</div></div>`;
...
document.getElementById(‘spotList’).innerHTML = res.map(s => ` <div class="spotItem ${s.unusable ? 'unusable' : s.occupied ? 'occupied' : 'free'}" onclick="openSpotDrawer('${s.id}')"> <div class="spotId">${s.id}</div> <div class="spotInfo"> <div class="spotPlate">${s.plate || (s.occupied ? '—' : s.unusable ? 'Inutilizzabile' : 'Libero')}</div> <div class="spotMeta">${s.zone}${s.since ? ' · ' + fmtDur(s.since) : ''}${s.damaged ? ' · ⚠️ Guasto' : ''}${s.unusable ? ' · 🚫 Inutilizzabile' : ''}${s.full ? ' · 🟡 Pieno' : ''}</div> </div> <span class="spotBadge ${s.unusable ? 'unusable' : s.occupied ? 'occ' : 'free'}">${s.unusable ? 'Inutilizzabile' : s.occupied ? 'Occupato' : 'Libero'}</span> </div>`).join(’’) || ‘<div class="emptyState">Nessun posto trovato</div>’;
}
...
let html = `<div class="drawerTitle">${modeIcon} Posto ${id}</div> <div style="font-size:12px;color:var(--muted);margin-bottom:14px">${sp.zone}</div>`;
...
if (canManage) {
html += `<label style="font-size:12px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.4px;display:block;margin-bottom:6px">${modeLabel} / Identificativo</label> <input class="inputField" id="drawerInput" type="text" placeholder="${mode === 'cassa' ? '042' : 'ABCD1234567'}" maxlength="15" oninput="this.value=this.value.toUpperCase()" onkeydown="if(event.key==='Enter')assignFromDrawer('${id}')"> <label class="checkLabel"><input type="checkbox" id="drawerFull" style="accent-color:var(--orange)"> 🟡 Piena/o (carico completo)</label> <button class="btnGreen" onclick="assignFromDrawer('${id}')">✓ Assegna ${modeLabel}</button>`;
} else {
...
if (canManageDamage) {
html += `<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)"> <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin-bottom:8px">Stato posto</div> <button class="btnUnusable" onclick="addUnusableDrawer('${id}')">🚫 Segna come inutilizzabile</button> </div>`;
}
} else {
html += `<div style="background:#ef444413;border:1px solid #ef444430;border-radius:10px;padding:10px 14px;margin-bottom:14px;font-weight:700;color:var(--red)">🔴 Occupato</div> <div class="infoRow"><span class="infoKey">${modeLabel}</span><span class="infoVal">${sp.plate || '—'}</span></div> <div class="infoRow"><span class="infoKey">Da</span><span class="infoVal">${fmtDate(sp.since)}</span></div> <div class="infoRow"><span class="infoKey">Durata</span><span class="infoVal">${fmtDur(sp.since)}</span></div> <div class="infoRow"><span class="infoKey">Utente</span><span class="infoVal" style="font-size:12px">${sp.user || '—'}</span></div> <div class="infoRow"><span class="infoKey">Stato cassa</span><span class="infoVal">${sp.full ? '🟡 Piena' : '🟢 Vuota'}</span></div> ${sp.damaged ? '<div class="infoRow"><span class="infoKey">Veicolo</span><span class="infoVal" style="color:var(--red)">⚠️ Guasto segnalato</span></div>' : ''} <div style="margin-top:14px"></div> ${sp.full ? `<button class="btnGreen" style="margin-bottom:8px" onclick="toggleFullDrawer('${id}',false)">🟢 Segna come Vuota</button>`:`<button class="btnOrange" style="margin-bottom:8px" onclick="toggleFullDrawer('${id}',true)">🟡 Segna come Piena</button>`}`;
if (canManage) {
...
if (canManageDamage) {
html += `<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)"> <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin-bottom:8px">Stato veicolo</div> ${sp.damaged ? `<button class="btnOrange" style="margin-bottom:8px" onclick="removeDamagedDrawer('${id}')">✓ Rimuovi segnalazione guasto</button>`:`<button class="btnOrange" style="margin-bottom:8px" onclick="addDamagedDrawer('${id}')">⚠️ Segna veicolo come guasto</button>`} <button class="btnUnusable" onclick="addUnusableDrawer('${id}')">🚫 Segna come inutilizzabile</button> </div>`;
}
}
html += `<button class="btnGray" style="margin-top:8px" onclick="closeDrawer()">Chiudi</button>`;
